...
function repoRelPath (repoDir) {
  const root = canonicalPath(runGit(['rev-parse', '--show-toplevel'], repoDir))
  const dir = canonicalPath(repoDir)
...
function materializeCommit (repoDir, sha, destDir) {
  ...
  const root = repoRelPath(repoDir).root
