...
function addPathsFromAction (paths, action) {
  ...
  const gate = action.if
  if (Array.isArray(gate)) {
    for (const c of gate) addPathsFromClause(paths, c)
  } else if (gate) {
    addPathsFromClause(paths, gate)
  }
}
...
function evalClause (clause, ctx) {
  ...
  if (Array.isArray(clause.all)) {
    const parts = clause.all.map((c) => withLabel(c, evalClause(c, ctx)))
    return {
  ...
  if (Array.isArray(clause.any)) {
    const parts = clause.any.map((c) => withLabel(c, evalClause(c, ctx)))
    const pass = parts.some((p) => p.pass)
    return {
      pass,
      error: pass ? null : (parts.find((p) => p.error)?.error || null),
      actual: parts,
      clause: { any: true }
    }
  }
  ...
  const needsValue = ['is', 'equals', 'above', 'below', 'to'].some((k) =>
    Object.prototype.hasOwnProperty.call(clause, k)
  )
  if (needsValue && (actual === undefined || actual === null)) {
    return {
      pass: false,
      error: 'missing',
      actual,
      path,
      expected: expectedOf(clause)
    }
  }
  let pass = false
...
function markPass (r) {
  if (!r) return '✗'
  if (r.error && !r.pass) return '✗'
  return r.pass ? '✓' : '✗'
}

function appendClauseLines (lines, r, indent) {
  if (!r) return
  lines.push(indent + markPass(r) + ' ' + (r.label || ''))
  const inner = r.clause && (r.clause.any || r.clause.all)
  if (!inner || !Array.isArray(r.actual)) return
  for (const part of r.actual) {
    appendClauseLines(lines, part, indent + '  ')
  }
}
...
function resultLabel (result) {
  return result === 'failure' ? 'Failed' : 'OK'
}

function formatStream (text) {
...
function actionStderr (action) {
  if (action && action.stderr) return String(action.stderr)
  const err = action && action.error ? String(action.error) : ''
  if (err.startsWith('stderr: ')) return err.slice(8)
  return ''
}

function appendRunActionLines (lines, action) {
  const extra = (action.args && action.args.length) ? ' ' + action.args.join(' ') : ''
  lines.push('  run ' + (action.file || '') + extra)
  lines.push('    stdout: ' + formatStream(action.stdout))
  lines.push('    stderr: ' + formatStream(actionStderr(action)))
  const err = action.error ? String(action.error) : ''
  if (err && !err.startsWith('stderr: ')) {
...
function formatVerboseLog (auto, record) {
  ...
  for (const b of record.branches || []) {
    ...
    for (const c of b.conditions || []) {
      appendClauseLines(lines, c, '    ')
    }
  }
  for (const a of record.actions || []) {
    if (a && a.type === 'run') appendRunActionLines(lines, a)
  }
  return lines.join('\n')
}
...
async function evaluateAutomation (auto, ctx) {
  ...
  if ((auto.condition || []).length && !cond.pass) {
    ...
    record.result = failed ? 'failure' : 'ok'
...
async function runAction (action, ctx) {
  ...
  if (action.if) {
    const gate = await evalListAll(Array.isArray(action.if) ? action.if : [action.if], ctx)
    if (!gate.pass) {
      return { ok: true, skipped: true, reason: 'if not met', if: gate.results }
    }
  }
  if (action.put) {
    try {
  ...
  if (action.delay != null) {
    ...
    if (ctx.skipDelay) return { ok: true, type: 'delay', ms, skipped: true, stop: true }
    try {
      await ctx.sleep(ms)
    } catch (err) {
      if (err && err.cancelled) {
        return { ok: true, type: 'delay', ms, cancelled: true, stop: true }
      }
      throw err
    }
    if (ctx.abort && ctx.abort.aborted) {
