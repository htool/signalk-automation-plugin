...
async function runAction (action, ctx) {
  ...
  if (action.run) {
    ...
    if (!ran.ok) {
      return { ok: false, type: 'run', file: spec.file, args: spec.args || [], error: ran.error, stdout: ran.stdout }
    }
    if (action.as) {
      ctx.setHelper(action.as, ran.value)
    }
    return { ok: true, type: 'run', file: spec.file, args: spec.args || [], value: ran.value, stdout: ran.stdout }
  }
